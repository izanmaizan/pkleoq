...
<head>
    ...
    <meta name="description" content="Laporan Produk - Sistem EOQ PT Wings Group Indonesia">
    <meta name="author" content="PT Wings Group Indonesia">

    <title>EOQ - Laporan Produk | PT Wings Group</title>
            ...
            <li class="nav-item active">
                ...
                <div id="report" class="collapse show" aria-labelledby="headingTwo" data-parent="#accordionSidebar">
                    <div class="bg-white py-2 collapse-inner rounded">
                        ...
                        <a class="collapse-item active" href="laporan_produk.php">
                ...
                <div class="container-fluid">
                        ...
                        <div class="d-flex align-items-center justify-content-between">
                            <div>
                                ...
                                <p class="mb-0 opacity-75">
                                    Laporan komprehensif data produk PT Wings Group Indonesia
                                </p>
                            </div>
                            <div class="d-none d-lg-block">
                                <i class="fas fa-file-chart-column fa-3x opacity-25"></i>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card card-stats">
                        ...
                        <div class="card-body">
                            ...
                            $sql_total = "SELECT SUM(p.stock * p.harga) as total_nilai FROM produk p";
                            $query_total = mysqli_query($conn, $sql_total);
                            $total_nilai = mysqli_fetch_assoc($query_total)['total_nilai'];

                            // Hitung produk dengan stok rendah
                            $sql_low_stock = "SELECT COUNT(*) as low_stock FROM produk WHERE stock < 10";
                            $query_low_stock = mysqli_query($conn, $sql_low_stock);
                            $low_stock_count = mysqli_fetch_assoc($query_low_stock)['low_stock'];
                            ?>
                                ...
                                <table class="table table-hover" id="tblProduk" style="width: 100%;">
                                    <thead>
                                        <tr>
                                            ...
                                            <th><i class="fas fa-calculator me-1"></i> Total Nilai</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php
                                        ...
                                        while ($data = mysqli_fetch_array($query)) :
                                            $total_nilai_item = $data['stock'] * $data['harga'];
                                        ?>
                                        <tr>
                                            <td><?= $no++; ?></td>
                                            <td><code class="text-primary"><?= $data['kode_produk']; ?></code></td>
                                            <td><span class="font-weight-bold"><?= $data['nama_produk']; ?></span></td>
                                            <td><span class="text-muted"><?= $data['nama_kategori']; ?></span></td>
                                            <td><span class="text-muted"><?= $data['nama_sup']; ?></span></td>
                                            <td><span class="text-success font-weight-bold">Rp
                                                    <?= number_format($data['harga'], 0, ',', '.'); ?></span></td>
                                            <td>
                                                <?php
                                                    $stockClass = 'stock-high';
                                                    if ($data['stock'] < 10) $stockClass = 'stock-low';
                                                    else if ($data['stock'] < 50) $stockClass = 'stock-medium';
                                                    ?>
                                                <span
                                                    class="stock-badge <?= $stockClass; ?>"><?= $data['stock']; ?></span>
                                            </td>
                                            <td><span class="text-muted"><?= $data['nama_satuan']; ?></span></td>
                                            <td><span class="text-warning font-weight-bold">Rp
                                                    <?= number_format($data['biaya_penyimpanan'], 0, ',', '.'); ?></span>
                                            </td>
                                            <td><span class="text-info font-weight-bold">Rp
                                                    <?= number_format($total_nilai_item, 0, ',', '.'); ?></span></td>
                                        </tr>
                                        <?php endwhile; ?>
                                    </tbody>
                                    <tfoot>
        ...
        $('#tblProduk').DataTable({
            ...
            buttons: [{
                ...
                },
                {
                    ...
                    title: 'Laporan Data Produk - PT Wings Group Indonesia',
                    messageTop: 'Laporan Data Produk\nTanggal: <?= date("d F Y"); ?>\nWaktu: <?= date("H:i"); ?> WIB'
                }
            ]
        });
