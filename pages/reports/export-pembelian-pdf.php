...
class PembelianPDF extends FPDF
{
  // Page header
  function Header()
  {
    ...
    $this->Cell(0, 10, 'LAPORAN TRANSAKSI PEMBELIAN', 0, 1, 'C');
...
// Create PDF instance
$pdf = new PembelianPDF('L', 'mm', 'LEGAL');
$pdf->AddPage();
...
while ($d = mysqli_fetch_array($sql)) {
  ...
  $pdf->CellFit(30, 6, $d['kode_transaksi'], 1, 0, 'C', $fill);
  ...
  $pdf->Cell(15, 6, number_format($d['stock_in']), 1, 0, 'C', $fill);
...
$pdf->Cell(0, 5, 'Total Nilai Pembelian: Rp ' . number_format($totalHargaKeseluruhan, 0, ',', '.'), 0, 1);
...
// Output PDF
$pdf->Output('D', 'Laporan_Pembelian_' . date('d-m-Y_His') . '.pdf');