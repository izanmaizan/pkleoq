...
$spreadsheet->getProperties()
    ...
    ->setSubject("Data Transaksi Penjualan")
    ->setDescription("Laporan transaksi penjualan PT Wings Group Indonesia");
...
if (isset($_GET['dari']) && isset($_GET['sampai']) && !empty($_GET['dari']) && !empty($_GET['sampai'])) {
    ...
    $query = "SELECT tp.id, tp.kd_transaksi, tp.tanggal, u.nama, tp.keterangan, 
    ...
    $filename = 'Laporan_Penjualan_' . date('d-m-Y', strtotime($dari)) . '_sd_' . date('d-m-Y', strtotime($sampai)) . '.xlsx';
    ...
    $filename = 'Laporan_Penjualan_Semua_' . date('d-m-Y') . '.xlsx';
}
...
while ($data = mysqli_fetch_assoc($datas)) {
    ...
    $sheet->setCellValue('H' . $i, (int)$data['stock_out']);
