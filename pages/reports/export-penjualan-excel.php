...
if (isset($_GET['dari']) && isset($_GET['sampai']) && !empty($_GET['dari']) && !empty($_GET['sampai'])) {
    ...
    $datas = mysqli_query(
        $conn,
        "SELECT tp.id, tp.kd_transaksi, tp.tanggal, u.nama, tp.keterangan, p.nama_produk, p.kode_produk, s.nama_satuan, tp.harga, tp.stock_out, tp.total_harga, sup.nama_sup
    ...
    $filename = 'Data Penjualan ' . date('d-m-Y', strtotime($dari)) . ' sd ' . date('d-m-Y', strtotime($sampai)) . '.xlsx';
} else {
    ...
    $filename = 'Data Penjualan Semua.xlsx';
}
