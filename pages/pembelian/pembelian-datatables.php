...
try {
    // Query untuk mengambil data pembelian (diperbaiki untuk GROUP BY)
    $sql = "SELECT 
        MIN(tp.id) as id, 
        tp.kode_transaksi, 
    ...
    while ($row = mysqli_fetch_assoc($query)) {
        ...
        $data[] = array(
            ...
            'aksi' => '<a href="javascript:void(0)" id="btn-detail" class="btn btn-sm btn-outline-info mr-2" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Detail" data-kode="' . $row['kode_transaksi'] . '" data-id="' . $row['id'] . '"><i class="fas fa-eye"></i></a>'
        );
    }
